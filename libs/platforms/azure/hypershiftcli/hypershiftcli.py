...
#        cluster_end_time = int(datetime.datetime.utcnow().timestamp())
        ...
        if rg_exist_out == "true":
            ...
            rg_destroy_code, rg_destroy_out, rg_destroy_err = self.utils.subprocess_exec("az group delete -y --resource-group " + cluster_info['resource_group_name'])
    ...
    def create_cluster(self, platform, cluster_name):
        ...
        rg_create_code, rg_create_out, rg_create_err = self.utils.subprocess_exec("az group create --location " + self.environment['azure_region'] + " --name rg-" + cluster_name, cluster_info["path"] + "/az_group_create.log")
        if rg_create_code != 0:
...
class HypershiftcliArguments(AzureArguments):
    def __init__(self, parser, config_file, environment):
        ...
        if not temp_args.mc_kubeconfig:
            parser.error("hcp-burner.py: error: the following arguments (or equivalent definition) are required: --mc-kubeconfig")
